...
exports.create = async (req, res, next) => {
    try {
        const { heading, readTime, description, categories, image, verified, newest, trending } = req.body;
        ...
        if (categories && Array.isArray(categories)) {
            categories.forEach(async (category) => {
                ...
                if (!existingCategory) {
                    return res.status(RESPONSES.BAD_REQUEST).json({ message: `category not found with categoryId ${category}` })
                }
            })
        }
        const newData = new ArticleModel({
            heading, readTime, description, categories, image, verified, newest, trending
        });
...
exports.findAll = async (req, res, next) => {
    try {
        ...
        if (category && !Array.isArray(category)) {
            const existingCategory = await CategoryModel.findById(category)
            if (!existingCategory) {
                return res.status(RESPONSES.BAD_REQUEST).json({ message: "category not found" })
            }
            query.categories = { $in: [existingCategory.id] }
        }

        else if (Array.isArray(category) && category.length > 0) {
            const existingCategories = await Promise.all(category.map(async item => await CategoryModel.findById(item.id)))
    ...
    try {
        ...
        ArticleModel.findByIdAndDelete(id, req.body)
            .then((data) => {
                ...
                return res.status(RESPONSES.NOT_FOUND).json({ message: 'unable to find article' })
            })
            .catch((err) => {
                return res
                    .status(RESPONSES.SERVER_ERROR)
